...
function targetAlive(s: GameState) {
  const base = 4 + (s.wave - 1) * 3.2 + s.level * 0.9;
  // 0 -> 1 across the wave: pressure builds toward the end of the wave
  const progress = Math.min(1, 1 - s.waveTimer / WAVE_LENGTH);
  const ramp = 0.55 + progress * 0.45;
  return Math.min(110, Math.round(base * ramp) + 3);
}
...
function spawnEnemy(s: GameState, forceElite = false, opts: SpawnOpts = {}) {
  ...
  const hp = Math.round(
    st.hp * 4.6 * (1 + (s.wave - 1) * 0.32) * breed.hpMul * (elite ? 3.4 : 1) * mul,
  );
  const e: Enemy = {
    x,
    y,
    ...
    tint: "",
    aura: breed.aura,
    damage: st.damage * breed.dmgMul * 1.15 * (1 + (s.wave - 1) * 0.05) * (opts.minion ? 0.8 : 1),
...
function waveBurst(s: GameState) {
  const n = Math.min(10, 1 + Math.round(s.wave * 0.7));
...
function killEnemy(s: GameState, e: Enemy) {
  ...
  if (e.role === "split" && !e.minion) {
    for (let i = 0; i < 3; i++) spawnMinion(s, e, "e_swarmer", 0.85);
...
export function update(s: GameState, input: Input, dt: number) {
  ...
  if (s.breather > 0) {
    ...
    if (s.spawnTimer <= 0 && alive < target) {
      ...
      s.spawnTimer = Math.max(0.45, 2.1 - s.wave * 0.06);
      const batch = 1 + Math.floor(s.wave / 8);
    ...
    switch (e.role) {
      ...
      case "brood": {
        ...
        if (e.cd <= 0 && e.spawned < (nest ? 14 : 5)) {
          e.cd = nest ? 1.1 : 3.6;
          e.spawned++;
          spawnMinion(s, e, nest && Math.random() < 0.35 ? "e_spitter" : "e_swarmer", nest ? 0.9 : 0.8);
        }
        break;
      }
      ...
      case "hydra":
        ...
        if (!e.didSplit && !e.minion && e.hp < e.maxHp * 0.5) {
          e.didSplit = true;
          for (let k = 0; k < 2; k++) {
            const c = spawnMinion(s, e, "e_hydra", 0.55);
            c.didSplit = true;
          }
...
export function render(ctx: CanvasRenderingContext2D, s: GameState, sprites: Sprites, time: number) {
  ...
  const drawEcho = (e: Echo) => {
    ...
    if (!e.dead) {
      drawGun(ctx, sprites, e.weapon, e.x, e.y - 30, e.aim, e.facing, 0, e.muzzle > 0);
      // echo hp bar
      ctx.globalAlpha = 0.8;
      ctx.fillStyle = "rgba(10,8,16,0.8)";
  ...
  if (!s.over) {
    ...
    drawGun(ctx, sprites, p.weapon, p.x, p.y - 30, p.aim, p.facing, recoil, s.muzzle > 0);
  }
  ctx.restore();
